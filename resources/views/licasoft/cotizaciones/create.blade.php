...
<!-- Modal -->
  ...
  <a href="http://localhost:8000/licasoft/"><button class="btn btn-block btn-danger">Volver</button></a>        
    ...
    {!!Form::open(array('url'=>'licasoft/cotizaciones','method'=>'POST','autocomplete'=>'off'))!!}
  {{Form::token()}}
...
{!!Form::close()!!}	
    ...
    <B>Hora Aprox Traslado:</B>
        ...
        <select  class="form-control" id="inputAmbulancia_id">
				...
				<option value="{{ $ambulancia['kmxlitro'] }}"> {{$ambulancia['modelo'] }} {{$ambulancia['marca'] }} {{$ambulancia['combustible'] }} {{$ambulancia['año'] }}</option>
				@endforeach
				</select>
            </div>
