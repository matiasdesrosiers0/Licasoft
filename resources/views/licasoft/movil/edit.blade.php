...
@section ('contenido')
	<div class="row">
		<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
			<h3>Editar movil Numero: {{ $movil->id_movil}}</h3>
			@if (count($errors)>0)
			...
			{!!Form::model($movil,['method'=>'PATCH','route'=>['licasoft.movil.update',$movil->id_movil]])!!}
            {{Form::token()}}
		   ...
		   <div class="form-group">
				...
				<option value="{{$ambulancia->id_ambulancia}}" selected> {{$ambulancia->modelo }}</option>
				@endforeach
				</select>
		...
		<div class="form-group">
				...
				<option value="{{$profesional->nombre }}" selected> {{$profesional['nombre'] }}</option>
				@endif
				@endforeach
				</select>
            </div>
